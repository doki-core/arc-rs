...
// CRC-32/ISO-HDLC of the grammar file: f0a8c008
...
#[derive(Debug, Clone)]
pub struct KeyNode {
    pub path: Vec<KeyNodeItem>,
}
#[allow(non_camel_case_types)]
#[derive(Debug, Clone)]
pub enum KeyNodeItem {
...
mod peginator_generated {
            ...
            mod closure {
                ...
                pub fn parse<'a, TT: ParseTracer>(
                    ...
                    let ParseOk { result: mut path, state } = parse_Whitespace(state, &mut *global)
                        .and_then(|ParseOk { state, .. }| parse_KeyNodeItem(state, &mut *global))
                        .map_inner(|result| vec![result])?;
                    Ok(ParseOk { result: path, state })
                }
                pub type Parsed = Vec<KeyNodeItem>;
            }
            #[inline(always)]
            pub fn parse<'a, TT: ParseTracer>(
                state: ParseState<'a>,
                ...
                let mut iterations: usize = 0;
                let mut state = state;
                let mut path: Vec<KeyNodeItem> = Vec::new();
                loop {
    ...
    pub(super) fn parse_KeyNode<'a, TT: ParseTracer>(
        ...
        let result = {
            let result = KeyNode_impl::parse(state, global)?.map(|r| super::KeyNode { path: r });
            Ok(result)
        };
        global.tracer.print_trace_result(&result);
        result
    }
    mod KeyNodeItem_impl {
        use super::*;
        #[inline(always)]
        pub fn parse<'a, TT: ParseTracer>(
            state: ParseState<'a>,
        ...
        pub type Parsed = Parsed__override;
        use super::KeyNodeItem as Parsed__override;
    }
    #[inline]
    pub(super) fn parse_KeyNodeItem<'a, TT: ParseTracer>(
        state: ParseState<'a>,
        global: &mut ParseGlobal<TT, ParseCache<'a>, ()>,
    ) -> ParseResult<'a, KeyNodeItem> {
        global.tracer.print_trace_start(&state, "KeyNodeItem");
        let result = {
            let result = KeyNodeItem_impl::parse(state, global)?;
            Ok(result)
        };
        global.tracer.print_trace_result(&result);
        result
    }
    impl PegPosition for KeyNodeItem {
            ...
            mod choice_0 {
                ...
                pub fn parse<'a, TT: ParseTracer>(
                    ...
                    let ParseOk { state, .. } = parse_Whitespace(state, &mut *global)
                        .and_then(|ParseOk { state, .. }| parse_character_literal(state, '('))
                        .discard_result()?;
                    let ParseOk { result: mut items, state } = part_1::parse(state, global)?;
                    let ParseOk { state, .. } = parse_Whitespace(state, &mut *global)
                        .and_then(|ParseOk { state, .. }| parse_character_literal(state, ')'))
                        .discard_result()?;
                    Ok(ParseOk { result: items, state })
